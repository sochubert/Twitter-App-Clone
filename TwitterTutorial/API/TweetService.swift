...
//  TwitterTutorial
//
//  Created by Minjae Lee on 2022/12/26.
//
...
struct TweetService {
    ...
    func uploadTweet(caption: String, type: UploadTweetConfiguration, completion: @escaping(DatabaseCompletion)) {
        ...
        case .reply(let tweet):
            ...
            REF_TWEET_REPLIES.child(tweet.tweetID).childByAutoId().updateChildValues(values) { (err, ref) in
                ...
                REF_USER_REPLIES.child(uid).updateChildValues([tweet.tweetID: ref.key], withCompletionBlock: completion)
            }
        }
    }
    ...
    func likeTweet(tweet: Tweet, completion: @escaping(DatabaseCompletion)) {
        ...
        if tweet.didLike {
            // Unlike data from firebase
            REF_USER_LIKES.child(uid).child(tweet.tweetID).removeValue { (err, ref) in
                REF_TWEET_LIKES.child(tweet.tweetID).removeValue(completionBlock: completion)
            }
        } else {
            // Like data to firebase
